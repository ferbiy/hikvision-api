...
export class Nvr extends Base {
    ...
    public async getWebsocketUrl(): Promise<string> {
        if (this.version === 2) {
            ...
            const result = `ws://${this.ip}:${this.wsPort}/?version=1.0&cipherSuites=1&token=${token}`;
            debugger
            return result
        } else {
            const result = `ws://${this.ip}:${this.wsPort}/?version=1.0&cipherSuites=1&sessionID=${this.session}`;
            debugger
            return result
        }
    }
    ...
    public async connect(): Promise<void> {
        ...
        // Make the same request as above but using Digest-Axios instead of Axios
        ...
        const response = await digestAuth.request({
            url: `http://${this.ip}/ISAPI/Security/sessionLogin?timeStamp=${Date.now()}`,
            method: 'post',
            data: XmlHandler.build({
                SessionLogin: {
                    userName: this.user,
                    password: p,
                    sessionID: params.sessionId,
    ...
    /**
     * Get the capability of RTSP authentication method
     */
    public async getRtspAuthMethod(): Promise<any> {
        const response = await this.request.get('/ISAPI/Security/RTSPCertificate/capabilities', {
            headers: this.headers
        });
        const parser = new XMLParser()
        const result = parser.parse(response.data);
        // const res = await XmlHandler.parser(response.data);
        debugger
        return result;
    }

    public async getRtsp(): Promise<any> {
        const response = await this.request.get('/ISAPI/System/Network/extension', {
            headers: this.headers
        });
        const json = await XmlHandler.parser(response.data);
        debugger
        return json;
    }
}
